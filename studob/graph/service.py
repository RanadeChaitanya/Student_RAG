...
class ConceptGraphService:
    def __init__(self, store: ConceptGraphStore, config: Settings, session_factory=None):
        self._store = store
        self._config = config
    ...
    async def get_related_concepts(self, concept_id: str, depth: int = 1) -> list[dict]:
        return [
            {"id": n.id, "display_name": n.display_name}
            for n in self._store.get_related_concepts(concept_id, depth)
        ]
    ...
    async def get_misconceptions(self, concept_id: str) -> list[dict]:
        ...
        try:
            ...
            from studob.database.engine import AsyncSession
            from studob.database.models import Misconception
