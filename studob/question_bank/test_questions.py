...
from studob.schemas.question import QuestionFilter, TestQuestionCreate, TestQuestionResponse
...
class TestQuestionService:
    ...
    async def get_questions_by_subject(
        self, subject: str, topic: str | None = None
    ) -> list[TestQuestionResponse]:
        async with self._session_factory() as session:
            session: AsyncSession
            conditions = [TestQuestion.subject == subject, TestQuestion.is_active]
            if topic:
                conditions.append(TestQuestion.topic == topic)
            result = await session.execute(select(TestQuestion).where(and_(*conditions)).limit(10))
            questions = result.scalars().all()
            return [TestQuestionResponse.model_validate(q) for q in questions]
