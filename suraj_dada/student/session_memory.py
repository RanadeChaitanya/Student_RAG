...
from suraj_dada.database.models import Attempt, Session, Student
from suraj_dada.exceptions import NotFoundError
from suraj_dada.logging_setup import get_logger
from suraj_dada.schemas.session import AttemptCreate, AttemptResponse, SessionResponse
...
class SessionMemoryService:
    ...
    async def get_recent_concepts(self, student_id: str, limit: int = 5) -> list[str]:
        async with self._session_factory() as session:
            ...
            seen = []
            for a in attempts:
                subtopic = getattr(a, "question_id", None)
